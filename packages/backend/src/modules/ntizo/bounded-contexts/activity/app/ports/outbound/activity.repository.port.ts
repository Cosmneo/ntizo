...
export interface ActivityRepositoryPort {
   ...
   *
   * `cursor` is an opaque string this port hands back as `ActivityPage.nextCursor`
   * and expects verbatim on the following call — callers must not construct
   * or parse it. A `cursor` that does not decode (malformed, truncated, or
   * from a client sending something else entirely) is **rejected**, not
   * silently treated as absent: the implementation throws
   * `CursorInvalidError` rather than degrading to the first page, because a
   * caller paginating by following `nextCursor` until it sees `null` could
   * otherwise loop forever on a corrupted token without ever finding out.
   */
  listForActor(params: {
    actorUserId: string;
    limit: number;
