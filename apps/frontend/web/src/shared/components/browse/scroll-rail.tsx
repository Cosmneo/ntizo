 ...
 *
 * The breakpoint is `sm` (640px), not `md`: every list this wraps already
 * switches its own column count at `sm` and up, and the rail must hand off to
 * that exact layout with nothing left over — a different cutoff here would
 * make the rail's grid and the section's own grid disagree for a width where
 * only one of them is supposed to be active.
 *
 * Renders a `div` by default; pass `as="ul"` for a list whose items are
 * `<li>`, so the rail never puts a list item under a non-list parent.
 *
...
export function ScrollRail({
  children,
  /** The element the rail renders as. `"ul"` for a list of `<li>` children. */
  as: Tag = "div",
  /** Desktop columns. Below `sm` this is ignored — it is one scrolling row. */
  columns,
  ...
  return (
    <Tag
      ...
      className={cn(
        ...
        "sm:mx-0 sm:grid sm:gap-x-6 sm:gap-y-6 sm:overflow-visible sm:px-0 sm:pb-0",
        grid,
