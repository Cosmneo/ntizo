 ...
 */
const resources = {
  en: { common: enCommon, auth: enAuth, provider: enProvider, admin: enAdmin, directory: enDirectory },
  pt: { common: ptCommon, auth: ptAuth, provider: ptProvider, admin: ptAdmin, directory: ptDirectory },
  es: { common: esCommon, auth: esAuth, provider: esProvider, admin: esAdmin, directory: esDirectory },
  de: { common: deCommon, auth: deAuth, provider: deProvider, admin: deAdmin, directory: deDirectory },
  fr: { common: frCommon, auth: frAuth, provider: frProvider, admin: frAdmin, directory: frDirectory },
  it: { common: itCommon, auth: itAuth, provider: itProvider, admin: itAdmin, directory: itDirectory },
  nl: { common: nlCommon, auth: nlAuth, provider: nlProvider, admin: nlAdmin, directory: nlDirectory }
};
...
void i18n
  ...
  .init({
    fallbackLng: "en",
    ns: ["common", "auth", "provider", "admin", "directory"],
    defaultNS: "common",
