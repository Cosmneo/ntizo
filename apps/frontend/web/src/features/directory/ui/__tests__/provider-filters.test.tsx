...
const { ProviderFilters } = await import("../provider-filters");

async function renderFilters(current: DirectorySearch) {
  const root = createRootRoute({ component: () => <ProviderFilters current={current} /> });
