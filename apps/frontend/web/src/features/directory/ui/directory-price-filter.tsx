 ...
 * person might type, and a link per keystroke would put a history entry between
 * them and the back button. So this one is a form — typed, then submitted — and
 * the submission is a navigation, which keeps the result a URL somebody can
 * send even though the control that produced it was not.
 *
 * The bound is on the business's cheapest published option, which is the same
 * number its card prints as "from". A business with nothing priced has no such
 * number and drops out of a bounded search rather than being treated as free.
 */
export function DirectoryPriceFilter({ current }: { current: DirectorySearch }) {
  ...
  // link and a cleared filter all change it without going through these boxes,
  // and boxes still showing the old numbers would be lying about what the list
