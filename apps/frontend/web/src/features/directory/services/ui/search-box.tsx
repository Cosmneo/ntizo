...
/**
 * The browse's search box.
 *
 * The one control on this page that cannot be a link: a link needs its
 * destination before the click, and this one is whatever the reader types.
 * So it is a real form, and submitting it navigates — which keeps the rest of
 * the page's contract intact. The term ends up in the URL, so a search is
 * still something you can send to somebody, reload, and undo with the back
 * button.
 *
 ...
 * search is an unindexed scan, so those are the expensive kind. Debouncing
 * would only make the same wrong thing happen less often.
 *
 ...
 */
export function SearchBox({ current }: { current: BrowseSearch }) {
  ...
  const go = (q: string) => void navigate({ to: "/services", search: browseSearch(current, { q }) });
  ...
  return (
    ...
    >
      ...
      {/* Clearing is its own button rather than only emptying the field: an
          ...
          the mismatch reads as the search being broken. It appears only when
          there is a submitted search to clear. */}
      {submitted ? (
        <button
          type="button"
          onClick={() => {
