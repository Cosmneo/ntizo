...
open below as its own entry (#126): whether an admin reading a private customer ↔ provider
...
## #126 — Admin oversight of a customer ↔ provider conversation still has no logging-or-consent decision
...
## #127 — `fromTheOtherSide` stayed a single copy; the cursor pagination helper did not
...
## #128 — `SUPPORT_REQUEST_OPENED` is raised per admin, synchronously inside the request
...
## #129 — An admin who is also a provider member answers their own request under two different `senderSide`s
...
## #130 — Frontend copy for the four notification types, and two inbox fields, are unfinished on the frontend side
...
## #131 — The attachment download route's admin check is a second database read on every non-participant lookup
...
## #132 — Migration 0035 is live on the shared dev database; the previously-deployed backend there cannot insert a message until this branch deploys
...
## #133 — `attachments.test.ts`'s `withPlatformRole` mocks `../admin-access` for the whole test process
...
## #134 — Communication's `bootstrap/index.ts` lost two inline comments when Task 7 wired the support commands
